...
std::vector<Vertex> Isolines::getIsolineVertices(Polygon polygon, float z) {
    ...
    if (a == b && a == 0) {
        ...
        float t = a / (a - b);
        Vertex v = Vertex();
        Vector3F direction = (polygon.vertices[1].position - polygon.vertices[0].position);
        Vector3F offset = direction * t;
...
void Isolines::setColors(Vector3F colorA, Vector3F colorB) {
    this->colorA = colorA;
}

void Isolines::setPosition(Vector3F position) {
    Placeable::setPosition(position);
}

void Isolines::setRotation(Vector3F rotation) {
    Placeable::setRotation(rotation);
}

void Isolines::setScale(Vector3F scale) {
